...
export class SimulateService extends Repository {
  private get preLoginFlowRequests(): Array<() => any> {
    return [
      () => this.client.account.readMsisdnHeader().catch(() => undefined), //() => this.client.account.readMsisdnHeader(),
      () => this.client.account.msisdnHeaderBootstrap('ig_select_app').catch(() => undefined), //() => this.client.account.msisdnHeaderBootstrap('ig_select_app'),
